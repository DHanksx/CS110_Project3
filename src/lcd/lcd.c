...
void LCD_ShowChar_Vertical(u16 x, u16 y, u8 num, u16 color) {
    u8 temp;
    u8 pos, t;
    u16 x0 = x;

    if (x > LCD_W - 16 || y > LCD_H - 16) return; // 检查是否超出屏幕范围

    num = num - ' '; // 获取偏移值
    for (pos = 0; pos < 16; pos++) {
        temp = asc2_1608[(u16)num * 16 + pos]; // 获取字体数据
        for (t = 0; t < 8; t++) {
            if (temp & 0x01) {
                LCD_DrawPoint(x, y + t, color); // 绘制像素点
            }
            temp >>= 1;
        }
        y++;
